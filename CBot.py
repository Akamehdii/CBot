            ...
            if action == "approve":
                ...
                if link:
                    ...
                    await query.edit_message_reply_markup(
                        reply_markup=InlineKeyboardMarkup(
                            [[InlineKeyboardButton(f"✅ توسط {admin_name} تایید شد", callback_data="done")]]
                        )
                    )
                else:
                    await context.bot.send_message(
                        chat_id=user_chat_id,
                        text="🎉 ثبت‌نامت تایید شد! به‌زودی اطلاعات نهایی برات ارسال می‌شه."
                    )
                    await query.edit_message_reply_markup(
                        reply_markup=InlineKeyboardMarkup(
                            [[InlineKeyboardButton(f"✅ توسط {admin_name} تایید شد (بدون لینک)", callback_data="done")]]
                        )
                    )
            else:
                await context.bot.send_message(
                    chat_id=user_chat_id,
                    text="⚠️ متاسفانه ثبت‌نامت تایید نشد. برای رویدادهای بعدی اقدام کن یا با پشتیبانی در تماس باش."
                )
                await query.edit_message_reply_markup(
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton(f"❌ توسط {admin_name} رد شد", callback_data="done")]]
                    )
                )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # free text ignored when expecting inline selection
...
async def finalize_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if GROUP_CHAT_ID:
            ...
            await context.bot.send_message(
                chat_id=GROUP_CHAT_ID, text=admin_txt, parse_mode='Markdown', reply_markup=buttons
            )
    except Exception as e:
...
def main():
    ...
    application.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex("^شروع مجدد 🔄$") & ~filters.Regex("^لغو عملیات ❌$"),
        handle_message
    ))
